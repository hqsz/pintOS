...
thread_init (void) 
{
  ...
  /* Initialize 4.4BSD scheduler variables. */
...
#ifndef USERPROG
  /* Thread aging when aging option on. */
  if (thread_prior_aging && !(thread_ticks % TIME_SLICE)){  
               ...
               thread_func *function, void *aux) 
{
  ...
  /* 4.4BSD scheduler variables must be inherited from parent thread. */
  t->nice = cur->nice;
...
bool is_open_file(const char *file){
  struct list_elem *e;
  struct thread *t;

  for(e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)){
    t = list_entry(e, struct thread, allelem);
    if(!strcmp(t->name, file) && t->status == THREAD_RUNNING) return true;
  }
  return false;
}
#endif
...
void update_recent_cpu(struct thread *t, void *aux UNUSED){
  t->recent_cpu = float_add_int(float_mul(float_div(float_mul_int(load_avg, 2), float_add_int(float_mul_int(load_avg, 2), 1)), t->recent_cpu), t->nice);
}

void update_priority(struct thread *t, void *aux UNUSED){
  t->priority = PRI_MAX - float_to_int(float_div_int(t->recent_cpu, 4)) - (t->nice * 2);
}
